...
if [ -f ~/liljr-autonomous/liljr_v90_omni.py ]; then
    cp ~/liljr-autonomous/liljr_v90_omni.py ~/liljr-system/brain/liljr_brain.py
else
    # Fallback: create minimal brain if repo not present
    cat > ~/liljr-system/brain/liljr_brain.py << 'PYEOF'
#!/usr/bin/env python3
...
class LilJrBrain:
    ...
    def load(self):
        ...
        return {"epoch": 0, "born": str(datetime.now()), "identity": "Lil Jr 2.0", "device": "S21", "knowledge": {}, "conversations": [], "tasks": [], "revenue": 0.0, "deployments": [], "status": "awake"}
    
    def save(self):
        self.data["epoch"] = self.epoch
        self.data["status"] = self.status
        self.data["last_active"] = str(datetime.now())
        with open(STATE, 'w') as f: json.dump(self.data, f, indent=2)
    
    def think(self, text):
        self.epoch += 1
        self.data["conversations"].append({"epoch": self.epoch, "time": str(datetime.now()), "input": text})
        self.save()
        return f"[EPOCH-{self.epoch}] Processed: {text[:60]}..."
    ...
    def health(self):
        return {"status": self.status, "epoch": self.epoch, "device": self.data.get("device", "S21")}
    ...
    def wake(self):
        ...
        self.save()
        log("REBOOT — RESUMING")
        return f"Back. Epoch {self.epoch}"
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            if cmd in ["exit","quit","shutdown"]: print(brain.sleep()); break
            elif cmd == "deploy": print(brain.deploy())
            elif cmd == "market": print(brain.market())
            elif cmd == "health": print(json.dumps(brain.health(), indent=2))
            elif cmd == "wake": print(brain.wake())
            elif cmd == "sleep": print(brain.sleep())
            else: print(brain.think(cmd))
        except KeyboardInterrupt: print(brain.sleep()); break
        except Exception as e: log(f"ERROR: {e}"); print(f"Error: {e}")
PYEOF
fi
chmod +x ~/liljr-system/brain/liljr_brain.py
