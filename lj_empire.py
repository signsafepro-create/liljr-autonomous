...
def main():
    ...
    # ═══ PERSONA ENGINE ═══
        ...
        if len(sys.argv) > 2:
            ...
            elif sub == 'switch' and len(sys.argv) > 3:
                print(json.dumps(pe.switch(sys.argv[3]), indent=2))
            elif sub == 'speak':
                print(pe.speak('Built something awesome', success=True))
            elif sub == 'train' and len(sys.argv) > 3:
                print(json.dumps(pe.train(' '.join(sys.argv[3:])), indent=2))
            elif sub == 'stats':
                print(json.dumps(pe.get_training_stats(), indent=2))
            elif sub == 'create' and len(sys.argv) > 3:
                print(json.dumps(pe.create_persona(sys.argv[3], ' '.join(sys.argv[4:]) if len(sys.argv) > 4 else 'Custom persona'), indent=2))
            else:
                print("PERSONA - Switch voices:")
                print('  persona list')
                print('  persona switch user')
                print('  persona switch masculine')
                print('  persona switch feminine')
                print('  persona switch professional')
                print('  persona switch andre')
                print('  persona speak')
                print('  persona train "your text here"')
                print('  persona stats')
        else:
            print("PERSONA - Switch voices:")
            print('  persona list - Show all personas')
            print('  persona switch NAME - Change voice')
            print('  persona speak - Test current voice')
            print('  persona train TEXT - Teach new words')
            print('  persona stats - Training data')
            print('')
